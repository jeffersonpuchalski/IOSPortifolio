...
struct Endpoint {
    ...
    /**
     Get a value from given plist in Main bundle.
     
     Find a plist with file name in main bundle, if we have a valid and match plist,
     transform in dictonary in hashable format (```[String : Any```), this allow
     we find value binded to parameter given key.
     - Parameters:
     - keyIdentifier: Key name to match with plist key.
     - typeOf: Generic type given for transform info.
     - Returns:
     A string value representation of scheme plist node.
     */
    static func getValue<T>(for keyIdentifier: String) -> T{
        let plistManager = PListManager()
        do{
            let dict = try plistManager.loadPlistToDictonary(for: "Urls")
            return dict[keyIdentifier] as! T
    ...
    /**
     ...
     transform in dictonary in hashable format (```[String : Any```), this allow
     we find value binded to parameter given key.
     - Parameter keyIdentifier: Key name to match with plist key.
     - Returns:
     A string value representation of scheme plist node.
     */
...
extension Endpoint {
    ...
    var url: URL? {
        ...
        components.scheme = Endpoint.getValue(for: "scheme")
        components.host = Endpoint.getValue(for: "host")
        // Validate if we have a valid port.
        let port: Int = Endpoint.getValue(for: "port")
        if port > 0 {
